...
(
	UserName NVARCHAR(100),
	DateStart date not null, -- thêm ngày bắt đầu tính lương
	Type bit NOT NULL, --sửa lại thành bit o/1 để check trạng thái tính lương (on/off)
	WORKDAY int,
	restday int,
	wagelevel float,
	bonus float,
	...
	primary key (username , DateStart, Type),
	foreign key (username) references dbo.account(username)
)
--Thêm dữ liệu cho Salary_demo
INSERT INTO [dbo].[salary_demo]
VALUES (N'UserName', )
drop table salary_demo
...
ALTER PROC INSERT_CUSTOMER @IDBILL INT , @NAMECUSTOMER NVARCHAR(100) , @SDT VARCHAR(12) , @DIACHI NVARCHAR(100)
AS
	BEGIN
...
ALTER PROC FIND_IDBILL @IDBILL INT
AS
...
alter PROC USP_GetListCustomerByPage
@PAGE INT 
AS
	BEGIN
